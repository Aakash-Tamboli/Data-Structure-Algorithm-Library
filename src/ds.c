...
{
    if (doublyLinkedListIterator == NULL)
        return false;
    if (doublyLinkedListIterator->node == NULL)
        return false;
    return true;
}
...
{
    if (doublyLinkedListReverseIterator == NULL)
        return false;
    if (doublyLinkedListReverseIterator->node == NULL)
        return false;
    return true;
}
...
{
    if (operationDetail)
        operationDetail->succ = false;
    AVLTree *avlTree;
    avlTree = (AVLTree *)malloc(sizeof(AVLTree));
    if (avlTree == NULL)
    {
        if (operationDetail)
            operationDetail->code = 2;
        return NULL;
    }
    if (predicate == NULL)
    {
        if (operationDetail)
            operationDetail->code = 6;
        return NULL;
    }
    avlTree->start = NULL;
    avlTree->size = 0;
...
void addToAVLTree(AVLTree *avlTree, void *ptr, OperationDetail *operationDetail)
{
    ...
    {
        if (operationDetail)
            operationDetail->code = oper.code;
        return;
    }
    j = avlTree->start;
    while (1)
...
void clearAVLTree(AVLTree *avlTree)
{
    ...
    while (1)
    {
        ...
        {
            if (t->right != NULL)
                pushOnStack(stack, (void *)&(t->right), &oper);
