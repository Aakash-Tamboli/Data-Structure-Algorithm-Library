...
void findingTheHeaviestElement(void *ptr, int lb, int ub, int es, OperationDetail *error, void *heavestElement, int (*p2f)(void *, void *))
{
    ...
    if (error == NULL)
    {
        if (isInvalid(ptr, &lb, &ub, &es, &err, p2f))
            return;
    }
    else
    {
        if (isInvalid(ptr, &lb, &ub, &es, error, p2f))
            return;
    }
    memcpy(heavestElement, (const void *)ptr + (lb * es), es);
    for (i = lb + 1; i <= ub; i++)
    {
        if (p2f(heavestElement, ptr + (i * es)) < 0)
            memcpy(heavestElement, (const void *)ptr + (i * es), es);
    }
    if (error)
    {
        error->code = 0;
...
void findingTheHeaviestElementIndex(void *ptr, int lb, int ub, int es, OperationDetail *error, int *heaviestElementIndex, int (*p2f)(void *, void *))
{
    ...
    *heaviestElementIndex = lb;
    for (i = lb + 1; i <= ub; i++)
    {
        if (p2f(ptr + ((*heaviestElementIndex) * es), ptr + (i * es)) < 0)
            *heaviestElementIndex = i;
    }
    if (error)
    {
        error->code = 0;
...
void findingTheLightestElement(void *ptr, int lb, int ub, int es, OperationDetail *error, void *lightestElement, int (*p2f)(void *, void *))
{
    ...
    for (i = lb + 1; i <= ub; i++)
    {
        if (p2f(lightestElement, ptr + (i * es)) > 0)
            memcpy(lightestElement, (const void *)ptr + (i * es), es);
    }
    if (error)
    {
        error->code = 0;
...
void insertionSortForDLL(DoublyLinkedList *doublyLinkedList, int lb, int ub, OperationDetail *error, int (*p2f)(void *, void *))
{
    ...
    if (error == NULL)
    {
        if (isInvalid((void *)doublyLinkedList, &lb, &size, &vEs, &err, dummy))
            return;
    }
    else
    {
        if (isInvalid((void *)doublyLinkedList, &lb, &ub, &vEs, error, dummy))
            return;
    }
    ...
    while (lbNode != NULL)
    {
        ...
        if (y == NULL)
            memcpy(doublyLinkedList->start->ptr, (const void *)c, doublyLinkedList->sizeOfOneElement);
        else
            memcpy(y->next->ptr, (const void *)c, doublyLinkedList->sizeOfOneElement);
        lbNode = lbNode->next;
    }
    free(c);
    if (error)
...
int findPartionPoint(void *ptr, int e, int f, int es, int (*p2f)(void *, void *), OperationDetail *err)
{
    ...
    while (e < f)
    {
        while (e < ub && p2f(ptr + (e * es), ptr + (lb * es)) <= 0)
            e++;
        while (p2f(ptr + (f * es), ptr + (lb * es)) > 0)
            f--;
        if (e < f)
        {
            a = ptr + (e * es);
    ...
    if (err.succ == true)
    {
        ...
        while (y > lb)
        {
            ...
            while (ri < y)
            {
                ...
                {
                    if (p2f(ptr + (lci * es), ptr + (rci * es)) > 0)
                        swi = lci;
                    else
                        swi = rci;
                }
...
void radixSort(void *x, int lb, int ub, int es, OperationDetail *error, int decision)
{
    ...
    if (error == NULL)
    {
        if (isInvalid(x, &lb, &ub, &es, &err, dummy))
            return;
    }
    else
    {
        if (isInvalid(x, &lb, &ub, &es, error, dummy))
            return;
    }
    if (sizeof(int) != es)
    {
    ...
    for (y = lb + 1; y <= ub; y++)
    {
        ...
        if (biggestNumber < elem)
            memcpy(&biggestNumber, (const void *)x + (y * es), es);
        if (smallestNumber > elem)
            memcpy(&smallestNumber, (const void *)x + (y * es), es);
    }
    ...
    if (smallestNumberDigit > biggestNumberDigit)
        largestDigit = smallestNumberDigit;
    else
        largestDigit = biggestNumberDigit;
    e = 1;
    f = 10;
...
void countSort(void *ptr, int lb, int ub, int es, OperationDetail *error, int decision)
{
    ...
    {
        OperationDetail err;
        if (isInvalid(ptr, &lb, &ub, &es, &err, dummy))
            return;
    }
    else
    {
        if (isInvalid(ptr, &lb, &ub, &es, error, dummy))
            return;
    }
    if (sizeof(int) != es)
    {
    ...
    if (i < j)
        largestElement = lightestElement;
    else
        largestElement = heaviestElement;
    if (largestElement < 0)
        largestElement = largestElement * -1; // now it becomes positive
...
void addressCalculationSort(void *ptr, int lb, int ub, int es, OperationDetail *error, int decision)
{
    ...
    if (biggestNumberDigit < smallestNumberDigit)
        largestDigit = smallestNumberDigit;
    else
        largestDigit = biggestNumberDigit;
    for (i = lb; i <= ub; i++)
